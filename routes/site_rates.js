...
router.post('/new', function(req, res) {
  ...
  Site.getSite(site, function(err, location) {
    if (err) {
      ...
      if (
        schedule.operating &&
        thisHour >= schedule.open &&
        thisHour < schedule.close
      ) {
      ...
      } else if (thisHour >= 1 && thisHour < 3) {
        //run this script to check for site devices between 1am and 3am
        ...
        if (device_ids.length > 0) {
          ids.forEach(function(id) {
            if (
              device_ids.some(function(device_id) {
                return device_id == id;
              })
            ) {
...
router.get('/:id/today', function(req, res) {
  ...
  var start = moment()
    .utcOffset(-4)
    .startOf('day'); //12 am this morning
  var end = moment()
    .utcOffset(-4)
    .endOf('day'); //1159 tonight
  ...
  Rate.get_stats(site, start, end, function(err, rates) {
    if (err || rates.length == 0) {
      ...
      if (rateObjsInHalfHr.length != 0) {
        ...
        var allPurchaseDurationsLastHalfHr = purchasesLastHalfHr.map(function(
          purchase
        ) {
        ...
        var totalTimeOfAllPurhcasesLastHalfHr = total(
          allPurchaseDurationsLastHalfHr
        );
        ...
        halfHrMoments.forEach(function(time, index) {
          ...
          var purchasesOccuring = purchasesLastHalfHr.filter(function(
            purchase
          ) {
            var purchaseEnd = moment(purchase.date).add(
              purchase.duration,
              'seconds'
            );

            var createdBetweenMinute = moment(purchase.date).isBetween(
              time,
              minuteTimeEnd
            );
            var endedBetween = purchaseEnd.isBetween(time, minuteTimeEnd);
            var endedAfter = minuteTimeEnd.isBetween(
              purchase.date,
              purchaseEnd
            );
  ...
  Rate.get_total_rates_for_range(site, start, end, function(err, rates) {
    ...
    if (diff > 0) {
      ...
      hrsInDay.forEach(function(hourMoment) {
        var purchasesThisHour = hoursOfPurchases.filter(function(
          purchaseMoment
        ) {
...
router.post('/:id/:range/averages', function(req, res) {
  ...
  Rate.get_avg_rates_for_range(site, start, end, function(err, rates) {
    ...
    if (diff > 0) {
      ...
      days.forEach(function(day, index) {
        var thisDayPurchases = rates
          .filter(function(rateObj) {
      ...
      var durations = rates.map(function(rateObject) {
      ...
      hrsInDay.forEach(function(hourMoment) {
        ...
        var purchaseDurationsThisHour = rates
...
router.post('/:id/:range/conversions', function(req, res) {
  ...
  Rate.get_stats(site, start, end, function(err, rates) {
    ...
    if (range == 'day') {
      ...
      hrsInDay.forEach(function(hour, index) {
        //array of transactions each hour
    ...
    var percentConversions = purchasesPerUnit
